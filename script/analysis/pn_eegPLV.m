...
% Code from: https://praneethnamburi.wordpress.com/2011/08/10/plv/
%
...
%   The matrix is actually numTimePoints x numChannels-1 x numChannels x 
%   numConditions. The last collumn of the second dimension is filled with
%   0
% 
%
%--------------------------------------------------------------------------
    ...
    for compareChannelCount = channelCount+1:numChannels
        ...
        for conditionCount = 1:numConditions
            plv(:, channelCount, compareChannelCount, conditionCount)...
                ...
                /sum(dataSelectArr(:, conditionCount));
        end
    end
end
plv = squeeze(plv);
