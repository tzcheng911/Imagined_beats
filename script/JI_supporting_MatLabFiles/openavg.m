function [avg] = openavgBioMAP(FILENAME)
...
% 	are reports.  Original openavg reported xmax+1 sample point. This problem was corrected in the original version of openavgdBioMAP in 2005
% 	but was never incorporated into openavg
% 	(2) Biologic uses non-integer sample rates.  openavg reads in only integer values for sample rates. Consequently, for BioMAP files the sample rate
% 	is recalculated based on the time window and number of points. This
% 	will only happen if there is one channel, and that channel is named "BioMAP" , or has "BioMAP in the name".
...
%	(4) openavg and openavgBioMAP are now identical.
