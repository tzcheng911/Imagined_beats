function events = processGblnTrigger(t,sig, varargin)
% processAudioTrigger
%
...
for iE = 1:length(onsetIdx)
    ...
    triggerLength = (thisTrigger(end) - thisTrigger(1)) * (1/.96); %Empirically, length is ~5% less than it should be--correct
    ...
    events(iE).sign = triggerSign;
    events(iE).amplitude = triggerAmplitude;
    events(iE).frequency = triggerFrequency;
    events(iE).length = triggerLength;
    events(iE).cycles = triggerCycles;
...
if iscell(opts.eventtype) && ( ~isempty(opts.eventduration) || ~isempty(opts.eventamplitude) || ~isempty(opts.eventfrequency) )
  ...
  for iE = 1:length(events)
    eventIdx = find(events(iE).length>=(durTgt(:,1)) ...
      & events(iE).length<=(durTgt(:,2)) ...
